...
class ArcGISClient:
    def __init__(self, config: AppConfig) -> None:
        ...
        self.client = httpx.Client(
            timeout=config.mbjb_request_timeout_seconds,
    ...
    def _request(self, url: str, params: dict[str, Any] | None = None) -> httpx.Response:
        ...
        for attempt in range(1, self.config.mbjb_retry_attempts + 1):
            try:
                ...
                last_error = exc
                if attempt == self.config.mbjb_retry_attempts:
                    break
    ...
    def fetch_layer_metadata(self, layer: ArcGISLayerConfig) -> JSONDict:
        return self.get_json(
            f"{self.config.mbjb_service_root}/{layer.base_url}",
            params={"f": "pjson"},
        )

    def fetch_count(self, layer: ArcGISLayerConfig) -> int:
        payload = self.get_json(
            f"{self.config.mbjb_service_root}/{layer.base_url}/query",
